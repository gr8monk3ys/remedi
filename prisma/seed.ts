...
async function main(): Promise<void> {
  ...
  for (let i = 0; i < pharmaceuticals.length; i += BATCH_SIZE) {
    ...
    console.log(`  Created pharmaceuticals ${i + 1} to ${Math.min(i + BATCH_SIZE, pharmaceuticals.length)}`)
  }
  ...
  for (let i = 0; i < allNaturalRemedies.length; i += BATCH_SIZE) {
    ...
    console.log(`  Created remedies ${i + 1} to ${Math.min(i + BATCH_SIZE, allNaturalRemedies.length)}`)
  }
  ...
  const validMappings = remedyMappings
    .filter((mapping) => {
      ...
      if (!pharmId) {
        console.warn(`  Warning: Pharmaceutical not found: ${mapping.pharmaceuticalName}`)
        return false
      }
      if (!remedyId) {
        console.warn(`  Warning: Remedy not found: ${mapping.naturalRemedyName}`)
        return false
      }
      return true
    })
  ...
  for (let i = 0; i < validMappings.length; i += BATCH_SIZE) {
    ...
    console.log(`  Processed mappings ${i + 1} to ${Math.min(i + BATCH_SIZE, validMappings.length)}`)
  }

  console.log(`Created ${createdMappings} mappings (${skippedMappings} duplicates skipped).`)
