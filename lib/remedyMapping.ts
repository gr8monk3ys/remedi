...
const INGREDIENT_MAPPINGS: Record<string, string[]> = {
  ...
  // Digestive health
  'omeprazole': ['aloe_vera', 'apple_cider_vinegar', 'deglycyrrhizinated_licorice'],
...
const DETAILED_REMEDIES: Record<string, DetailedRemedy> = {
  ...
  'turmeric': {
    ...
    usage: 'Turmeric can be used in cooking, taken as a supplement, or made into a paste for topical application.',
    dosage: 'The recommended dosage is typically 500-2,000 mg of turmeric extract per day.',
    precautions: 'May interact with blood thinners, diabetes medications, and acid-reducing medications.',
    scientificInfo: 'The active compound in turmeric, curcumin, inhibits inflammatory enzymes similar to NSAIDs.',
    references: [
      { 
  ...
  'ginger': {
    ...
    scientificInfo: 'Ginger contains compounds called gingerols and shogaols that have anti-inflammatory effects.',
    references: [
      { 
  ...
  'valerian_root': {
    ...
    imageUrl: 'https://images.unsplash.com/photo-1597362925123-77861d3fbac7?w=400&h=400&fit=crop',
    category: 'Herbal Remedy',
    matchingNutrients: ['Valerenic acid', 'Isovaleric acid'],
    similarityScore: 0.75,
    usage: 'Typically consumed as a tea, tincture, or in capsule form before bedtime.',
    dosage: '300-600mg of valerian root extract or 2-3g of dried root 1-2 hours before bedtime.',
    precautions: 'May cause drowsiness; avoid driving or operating machinery. Not recommended with alcohol or sedatives.',
    scientificInfo: 'Valerian may increase GABA levels in the brain, which helps regulate nerve cells and has a calming effect.',
    references: [
      { 
 ...
 */
export async function findNaturalRemediesForDrug(drug: ProcessedDrug): Promise<NaturalRemedy[]> {
  ...
  if (matchedRemedyIds.size === 0) {
    ...
    if (matchedRemedyIds.size === 0) {
      ...
      for (const [category, remedyIds] of Object.entries(CATEGORY_MAPPINGS)) {
        if (drug.category.includes(category) || category.includes(drug.category)) {
          remedyIds.forEach(id => matchedRemedyIds.add(id));
        }
      }
    }
