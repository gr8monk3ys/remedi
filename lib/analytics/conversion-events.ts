...
export type ConversionEventType = (typeof CONVERSION_EVENT_TYPES)[keyof typeof CONVERSION_EVENT_TYPES]
...
export async function trackConversionEvent(params: {
  ...
  const { userId, sessionId, eventType, eventSource, planTarget, metadata } = params
...
export async function getConversionMetrics(params: {
  ...
  const [promptsShown, promptsClicked, checkoutsStarted, checkoutsCompleted] = await Promise.all([
    prisma.conversionEvent.count({
      where: { ...whereClause, eventType: CONVERSION_EVENT_TYPES.UPGRADE_PROMPT_SHOWN },
    }),
    prisma.conversionEvent.count({
      where: { ...whereClause, eventType: CONVERSION_EVENT_TYPES.UPGRADE_PROMPT_CLICKED },
    }),
    prisma.conversionEvent.count({
      where: { ...whereClause, eventType: CONVERSION_EVENT_TYPES.CHECKOUT_STARTED },
    }),
    prisma.conversionEvent.count({
      where: { ...whereClause, eventType: CONVERSION_EVENT_TYPES.CHECKOUT_COMPLETED },
    }),
  ])

  const promptToClickRate = promptsShown > 0 ? (promptsClicked / promptsShown) * 100 : 0
  const clickToCheckoutRate = promptsClicked > 0 ? (checkoutsStarted / promptsClicked) * 100 : 0
  const checkoutCompletionRate = checkoutsStarted > 0 ? (checkoutsCompleted / checkoutsStarted) * 100 : 0
  const conversionRate = promptsShown > 0 ? (checkoutsCompleted / promptsShown) * 100 : 0
...
export async function getTrialMetrics(params: {
  ...
  const [trialsStarted, trialsConverted, trialsExpired] = await Promise.all([
    prisma.conversionEvent.count({
      where: { ...whereClause, eventType: CONVERSION_EVENT_TYPES.TRIAL_STARTED },
    }),
    prisma.conversionEvent.count({
      where: { ...whereClause, eventType: CONVERSION_EVENT_TYPES.TRIAL_CONVERTED },
    }),
    prisma.conversionEvent.count({
      where: { ...whereClause, eventType: CONVERSION_EVENT_TYPES.TRIAL_EXPIRED },
    }),
  ])
...
> {
  ...
  return events.map((e: { id: string; eventType: string; eventSource: string | null; planTarget: string | null; createdAt: Date; metadata: unknown }) => ({
    ...e,
...
> {
  ...
  return events.map((e: { id: string; userId: string | null; sessionId: string | null; eventType: string; eventSource: string | null; planTarget: string | null; createdAt: Date; metadata: unknown }) => ({
    ...e,
    metadata: e.metadata as Record<string, unknown> | null,
  }))
}
