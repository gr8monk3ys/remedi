...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!validation.success) {
      const errorMessage = validation.error.issues[0]?.message || "Invalid query parameter";
    ...
    log.debug('Query processed', { original: query, processed: processedQuery });
    ...
    if (dbPharmaceuticals.length > 0) {
      ...
      log.info('Found pharmaceutical in database', { name: pharmaceutical.name });
    ...
    if (drugResults.length > 0) {
      ...
      log.info('Found pharmaceutical from FDA API', { name: pharmaceutical.name });
      ...
      if (remedies.length > 0) {
        log.info('Found remedies using mapping algorithm', { count: remedies.length });
        await saveHistory(remedies.length);
        const processingTime = Date.now() - startTime;
        return NextResponse.json(
          successResponse(remedies, {
