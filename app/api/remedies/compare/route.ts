...
const MOCK_REMEDIES: Record<string, CompareRemedy> = {
  ...
  '104': {
    ...
    dosage: '1-2g powder, 1-2 teaspoons fresh, or 400-500mg extract 2-3x daily.',
...
function toCompareRemedy(
  ...
  // Parse references - they might be strings or Reference objects
  let parsedReferences: Reference[] = []
  if (remedy.references && Array.isArray(remedy.references)) {
    parsedReferences = remedy.references.map((ref) => {
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    for (const dbRemedy of dbRemedies) {
      ...
      const pharmaceuticals = dbRemedy.pharmaceuticals.map((mapping: {
