...
async function handleCheckoutSessionCompleted(
  ...
  if (!userId || !subscriptionId) {
    log.error('Missing userId or subscriptionId in session', null, { sessionId: session.id })
    return
  }
  ...
  await prisma.subscription.upsert({
    ...
    create: {
      ...
      currentPeriodStart,
      currentPeriodEnd,
      startedAt: new Date((stripeSubscription.start_date ?? Date.now() / 1000) * 1000),
    },
    update: {
      stripeSubscriptionId: subscriptionId,
      customerId,
...
async function handleSubscriptionUpdated(subscription: Stripe.Subscription) {
  ...
  await prisma.subscription.update({
    ...
    data: {
      ...
      status,
      priceId,
      interval: subscriptionItem?.price.recurring?.interval || dbSubscription.interval,
      currentPeriodStart,
      currentPeriodEnd,
  ...
  log.info('Subscription updated', { subscriptionId, status, plan: plan || dbSubscription.plan })
}
