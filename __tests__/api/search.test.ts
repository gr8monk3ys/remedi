...
vi.mock('@/lib/rate-limit', () => ({
  withRateLimit: vi.fn().mockResolvedValue({ allowed: true, result: { success: true } }),
  RATE_LIMITS: { search: { limit: 30, window: 60, identifier: 'search' } },
}));

import { searchPharmaceuticals, getNaturalRemediesForPharmaceutical, upsertPharmaceutical } from '@/lib/db';
...
describe('GET /api/search', () => {
  ...
  describe('Query Parameter Validation', () => {
    ...
    it('should accept valid query parameters', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/search?query=ibuprofen');
      const response = await GET(request);
  ...
  describe('Database Search (Tier 1)', () => {
    it('should return remedies from database when pharmaceutical is found', async () => {
      ...
      vi.mocked(getNaturalRemediesForPharmaceutical).mockResolvedValue(mockRemedies as any);
    ...
    it('should handle database returning no results and fallback to FDA API', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/search?query=aspirin');
      const response = await GET(request);
  ...
  describe('OpenFDA API Search (Tier 2)', () => {
    it('should use FDA API when database has no results', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/search?query=melatonin');
      const response = await GET(request);
    ...
    it('should cache FDA results in database', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/search?query=vitamin-d');
      await GET(request);

      expect(upsertPharmaceutical).toHaveBeenCalledWith(expect.objectContaining({
        name: 'Vitamin D',
        fdaId: 'fda-cache-test',
      }));
    });
  ...
  describe('Mock Data Fallback (Tier 3)', () => {
    it('should use fuzzy search on mock data when database and FDA fail', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/search?query=unknown-drug');
      const response = await GET(request);
      const json = await response.json();
  ...
  describe('Query Processing', () => {
    it('should handle queries with common suffixes', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/search?query=ibuprofen+tablet');
      await GET(request);
    ...
    it('should handle spelling variants', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/search?query=ibuprofin');
      await GET(request);
  ...
  describe('Error Handling', () => {
    it('should handle database errors and return 500', async () => {
      vi.mocked(searchPharmaceuticals).mockRejectedValue(new Error('Database error'));

      const request = new NextRequest('http://localhost:3000/api/search?query=test');
      const response = await GET(request);
      const json = await response.json();

      // Database errors should return 500 with error structure
  ...
  describe('Response Format', () => {
    it('should return array of remedy objects with correct structure', async () => {
      ...
      vi.mocked(getNaturalRemediesForPharmaceutical).mockResolvedValue([mockRemedy] as any);
