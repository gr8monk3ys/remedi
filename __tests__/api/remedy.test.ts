...
describe('GET /api/remedy/[id]', () => {
  ...
  describe('Valid Remedy ID', () => {
    it('should return remedy details from database when found', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/remedy/test-remedy-id');
    ...
    it('should return remedy with all expected fields', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/remedy/full-remedy');
  ...
  describe('Remedy Not Found', () => {
    it('should return 404 when remedy is not in database', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/remedy/nonexistent-id');
    ...
    it('should fallback to mock data when database returns null', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/remedy/turmeric');
  ...
  describe('ID Format Validation', () => {
    it('should handle UUID format IDs', async () => {
      ...
      const request = new NextRequest(`http://localhost:3000/api/remedy/${uuidId}`);
    ...
    it('should handle slug format IDs', async () => {
      ...
      const request = new NextRequest(`http://localhost:3000/api/remedy/${slugId}`);
  ...
  describe('Error Handling', () => {
    it('should return 500 on database error', async () => {
      vi.mocked(getNaturalRemedyById).mockRejectedValue(new Error('Database connection failed'));

      const request = new NextRequest('http://localhost:3000/api/remedy/test-id');
    ...
    it('should handle malformed IDs gracefully', async () => {
      ...
      const request = new NextRequest(`http://localhost:3000/api/remedy/${encodeURIComponent(malformedId)}`);
  ...
  describe('Related Remedies', () => {
    it('should include related remedies in response', async () => {
      const mockRemedy = {
        ...
        benefits: [],
        relatedRemedies: [
          { id: 'related-1', name: 'Related Remedy 1', reason: 'Similar properties' },
          { id: 'related-2', name: 'Related Remedy 2', reason: 'Alternative option' },
        ],
        evidenceLevel: 'Strong',
        createdAt: new Date(),
        updatedAt: new Date(),
      ...
      const request = new NextRequest('http://localhost:3000/api/remedy/main-remedy');
  ...
  describe('Scientific References', () => {
    it('should include scientific references in response', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/remedy/scientific-remedy');
