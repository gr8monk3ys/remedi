...
vi.mock('@/lib/authorization', () => ({
  ...
  verifyResourceOwnership: vi.fn().mockResolvedValue({ authorized: true, error: null }),
}));
...
describe('/api/favorites', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(verifyOwnership).mockResolvedValue({ authorized: true, error: null });
    vi.mocked(verifyResourceOwnership).mockResolvedValue({ authorized: true, error: null });
  });
  ...
  describe('GET /api/favorites', () => {
    it('should return list of favorites', async () => {
      ...
      expect(getFavorites).toHaveBeenCalledWith(mockSessionId, undefined, undefined);
    });
    ...
    it('should check if remedy is favorited', async () => {
      ...
      expect(isFavorite).toHaveBeenCalledWith('remedy-1', mockSessionId, undefined);
    });

    it('should return collection names', async () => {
      vi.mocked(getCollectionNames).mockResolvedValue(['Pain Relief', 'Sleep Aids']);
    ...
    it('should filter favorites by collection name', async () => {
      ...
      expect(response.status).toBe(200);
      expect(getFavorites).toHaveBeenCalledWith(mockSessionId, undefined, 'Pain Relief');
    });
  ...
  describe('POST /api/favorites', () => {
    ...
    it('should handle duplicate favorites', async () => {
      vi.mocked(addFavorite).mockRejectedValue(new Error('Unique constraint failed'));
    ...
    it('should verify ownership before adding', async () => {
      ...
      vi.mocked(verifyOwnership).mockResolvedValue({ authorized: false, error: mockError });
